...
/**
 * Abstraction over system prompt construction for a coding topic, tutor persona, and user developer profile.
 *
...
interface TopicPromptProvider {
    /**
     * Builds the system prompt for the given [topic], [persona], and optional [developerProfile].
     */
...
class DefaultTopicPromptProvider
    ...
    constructor() : TopicPromptProvider {
        override fun buildSystemPrompt(
            ...
            val developerDirectives =
                developerProfile
                    ?.formatPromptDirectives()
                    ?.takeIf { it.isNotBlank() }
                    ?.let { "\n\n$it" }
                    .orEmpty()

            return """$personaInstructions$developerDirectives
                |
